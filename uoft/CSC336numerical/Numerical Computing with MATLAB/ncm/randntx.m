...
% an old version of RANDN to within roundoff error.
% Usage:
%    randntx                 1-by-1
%    randtnx(n)              n-by-n
%    randntx(m,n)            m-by-n
...
%    randuni(j)
   ...
   jsr = bitxor(jsr,bitshift(jsr,13,'uint32'));
   jsr = bitxor(jsr,bitshift(jsr,-17,'uint32'));
   jsr = bitxor(jsr,bitshift(jsr,5,'uint32'));
   m = mod(icng+jsr,2^32);
