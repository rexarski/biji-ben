...
shg
clf reset
set(gcf,'numbertitle','off','menubar','none','color','white', ...
    'name','Numerical Computing with MATLAB','inverthardcopy','off')
if verLessThan('matlab','8.4')
    cmap = hot(16);
else
    cmap = parula(16);
end
colormap([white(1); jet(13); gray(8); cmap; copper(16)])
funs = cell(24,1);
for k = 1:24
   if k <= 19 
      p = rem(k-1,5);
   ...
   switch k
      ...
      case 22, f = @xncmgui; text(.1,.4,'helpwin ncmgui'), box on
      case 23, f = @xncm; text(.1,.4,'helpwin ncm'), box on
      case 24, f = @xclose; text(.1,.4,'close'), box on
   end
   set(ax,'xtick',[],'ytick',[]);
   if k <= 19 
...
axis off
set(gcf,'name','Numerical Computing with MATLAB') 
...
text(-1.10,0,'A','color','black')
text(-0.65,0,'\spadesuit','color','black')
patch(x+.7,y,offwhite)
text(0.25,0,'J','color','red')
text(0.65,0,'\heartsuit','color','red')
...
if verLessThan('matlab','8.4')
   ...
   imshow(imread('ncmlogo.png'))
   set(gcf,'color',[0 0 .25])
end
