    ...
    def is_heap(self, index=1):
        ...
        if self.is_empty():
            ...
            if right < len(self.items):
                ...
                if index_item > left_item and index_item > right_item:
                    l = self.is_heap(left)
                    r = self.is_heap(right)
...
def build_heap_2(lst):
    ...
    new_heap = Heap()
    my_lst = lst[:]
    new_heap.items += my_lst
    index = len(new_heap.items) - 1
    for i in range(index):
        ind = index - i
        new_heap.bubble_down(ind)
    return new_heap    
