...
# A tree has zero or more subtrees, each of which is a tree. (This is where trees and lists differ. You can think of a list as atree that only ever has zero or one subtrees.)
...
class Tree:
    ...
    # every non-emtpy tree has a root node (which is generally drawn at the top), connected to zero or more subtrees.
    ...
    # the parent of a node is the one immediately above and connected to it; each node has one parent, except the root, which has no parent.
    
    # the ancestor of a node are its parent, the parent of its parent, etc.
