...
df = pd.read_csv('I1-2007.csv',sep=',')
...
# mu_att = pymc.Normal('mu_att', 0, 0.0001, value=0)
# mu_def = pymc.Normal('mu_def', 0, 0.0001, value=0)
...
mcmc.sample(iter=200000, burn=40000, thin=20)
# mcmc.sample(iter=1000000, burn=100000, thin=1000)
...
fig.savefig('I1-2007-image/95hpd-att.png')

# VISUALIZATION of average defense effect vs average attack effect

df_observed = pd.read_csv('I1_2007_results.csv')
...
ax.set_title('Attack vs Defense avg effect: 07-08 Series A')
ax.set_xlabel('Avg attack effect')
ax.set_ylabel('Avg defense effect')
ax.legend()
fig.savefig('I1-2007-image/att-vs-def.png')
...
def simulate_season():
    ...
    season['home_theta'] = season.apply(lambda x: math.exp(x['home'] +
                                                           x['att_home'] +
                                                           x['def_away']),
                                        axis=1)
    # season['away_theta'] = season.apply(lambda x: math.exp(x['intercept'] +
    season['away_theta'] = season.apply(lambda x: math.exp(x['att_away'] +
                                                           x['def_home']),
                                        axis=1)
...
sim1 = create_season_table(simulate_season())
print(sim1)
sim1.to_csv("italia-sim1.csv")

simuls = simulate_seasons(1000)

fig, ax = plt.subplots()
ax = simuls.points[simuls.team == 'Roma'].hist(figsize=(7,5))
median = simuls.points[simuls.team == 'Roma'].median()
ax.set_title('Roma: 2007-2008 points, 1000 simulations')
ax.plot([median, median], ax.get_ylim())
plt.annotate('Median: %s' % median, xy=(median + 1, ax.get_ylim()[1]-10))
fig.savefig('I1-2007-image/med-pts.png')

fig, ax = plt.subplots()
ax = simuls.gf[simuls.team == 'Roma'].hist(figsize=(7,5))
median = simuls.gf[simuls.team == 'Roma'].median()
ax.set_title('Roma: 2007-2008 goals for, 1000 simulations')
ax.plot([median, median], ax.get_ylim())
plt.annotate('Median: %s' % median, xy=(median + 1, ax.get_ylim()[1]-10))
fig.savefig('I1-2007-image/med-gf.png')
...
fig.savefig('I1-2007-image/90CI-gf.png')

# confirmation of shrinkage
...
fig.savefig('I1-2007-image/simtop4.png')
